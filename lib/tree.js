...
module.exports = {
  ...
  buildNpmTree (nodeModulesTree, yarnObject) {
    const versions = Object.keys(yarnObject)
    .reduce((tree, module) => {
      const results = module.match(/([^@]+)@(.*)/)
      const name = module[0] === '@' ? `@${results[1]}` : results[1]
